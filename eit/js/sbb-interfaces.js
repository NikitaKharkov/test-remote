...
var baseUrl = {
	logos: thisdomain + '/images/logos/'
};

var ep = {};
var isDMP=false;
var dmpDomain = "auth.dynamed.com";
var dmpPath = "login.aspx";
var defaultDomain ="search.ebscohost.com";
var defaultPath ="login.aspx";
...
function ViewModel(configJson) {
		...
		var objToExtend = {
			...
			authMethods: {
				multiSelectable: [
					...
					authMethodMulti('url', 'Referring URL'),
					authMethodMulti('geo', 'Geolocation')
				],
				singleSelect: [
			...
			layout: {
				...
				logo: ko.observable(thisdomain+'/eit/images/ebscohost.gif'),
	...
	self.buildUrl = function (database) {
		var selectedInterface = self.selectedInterface,
			...
			interfaceName = (selectedInterface().name === 'edspub-live') ? 'pfi-live' : selectedInterface().name;
		isDMP = (selectedInterface().name === 'dmp-live') ? true : false;
		if (isDMP){
			defaultDomain = dmpDomain;
			defaultPath = dmpPath;
			var url = beginning +
				"/"+ defaultPath +"?" +
				'site=' + interfaceName +
		...
		defaultDomain ="search.ebscohost.com";
		defaultPath ="login.aspx";
		}

		var url = beginning +
			"/"+ defaultPath +"?" +
			((database) ? '' : 'direct=true&scope=site&type=' + selectedInterface().searchType) +
			'&site=' + interfaceName +
			databasesString +
			getProfileString() +
	...
	//Add limiters to Search box code
	...
	//Add Authtype to Search box code
	var getAuthTypeString = function () {
		...
		// 7-14-2017 .. adds geocustid if geo is a selected auth method
		if (selectedMethods.indexOf("geo") > -1) {
			methodsString += '&geocustid=' + custId;
		}
	...
	self.buildSearchBox = function (preview) {
		...
		'<script src="'+thisdomain+'/eit/scripts/ebscohostsearch.js' + '" type="text/javascript"></script>\n';
			...
			*/
			...
			if (ep.ebscohostsearchJS) {	// yes, it's cached
				...
				$.ajax({
					url: thisdomain+'/eit/scripts/ebscohostsearch.js',
			...
			$.each(limiters, function () {
				...
				switch(this.type()) {
					...
					case 'catalogOnly': self.limiters.catalogOnly = state;
						break;
					case 'IROnly': self.limiters.IROnly = state;
						break;
				}
			});
		}
	...
	*/
	self.areLimitersSelected = function () {
		...
		if(self.selectedInterface().name === 'dme-live'|| self.selectedInterface().name === 'eon-live' || self.selectedInterface().name === 'dmp-live')
			return false;
		else
		{
			var limiters = self.selectedInterface().limiters;
...
*/
			...
			//US6415: For Dynamed & Flipster making visible false
			var isDynamed = (viewModel.selectedInterface().name === 'dme-live' || viewModel.selectedInterface().name === 'dmp-live');
			...
			/* [a.hazen 3.20.2017] No longer beta
			...
			*/
			...
			authMethodsMulti: function () {
				return {
					foreach: viewModel.selectedInterface().authMethods.multiSelectable
				};
			},
			...
			authCustIdContainer: function () {
				return {
					/* 7-14-2017 .. MOVED TO JQUERY ON MAIN SBB.PHP PAGE
				visible: viewModel.authMethodSingle() === 'custuid' ||
				viewModel.authMethodSingle() === 'cpid' ||
				viewModel.authMethodSingle() === 'sso' ||
				viewModel.authMethodSingle() === 'shib' ||
				viewModel.authMethodSingle() === 'athens' */
				};
			},
			authCustId: function () {
			...
			getUrl = function () {
				...
				if (isEhost || isEli) {
					switch (url) {
						...
						case (baseUrl.logos + 'transparent_10.png'):
							return 'img/sbb/PreviewCustom.gif';
					}
				}
				return url;
			};
			...
			disciplineDisplayTypeNone: function () {
				return {
					value: 'none',
			...
			buildSearchbox: function () {
				return {
					...
					!viewModel.profile.groupId())) {
						...
						if ($('#geo:checked').length && !viewModel.selectedInterface().custId().length && $("#customerId").val()=="") {
							errorMessage += 'Geolocation requires your Cust ID to authenticate.\n';
							errorFlag = true;
						}
						if ($('#custuid:checked').length && !viewModel.selectedInterface().custId().length && $("#customerId").val()=="") {
							errorMessage += 'Patron ID requires your Cust ID to authenticate.\n';
							errorFlag = true;
						}
						if ($('#cpid:checked').length && !viewModel.selectedInterface().custId().length && $("#customerId").val()=="") {
							errorMessage += 'Patterned ID requires your Cust ID to authenticate.\n';
							errorFlag = true;
						}
						if ($('#shib:checked').length && !viewModel.selectedInterface().custId().length && $("#customerId").val()=="") {
							errorMessage += 'Shibboleth requires your Cust ID to authenticate.\n';
							errorFlag = true;
						}
						if ($('#sso:checked').length && !viewModel.selectedInterface().custId().length && $("#customerId").val()=="") {
							errorMessage += 'Single Sign-on requires your Cust ID to authenticate.\n';
							errorFlag = true;
						}
						if ($('#athens:checked').length && !viewModel.selectedInterface().custId().length && $("#customerId").val()=="") {
							errorMessage += 'Athens requires your Cust ID to authenticate.\n';
							errorFlag = true;
						}
					...
					!isUrl(viewModel.logo())) {
						...
						if (errorFlag) {
                            ...
                            viewModel.buildSearchBox();
														...
														//console.log("full object:", this.selectedInterface().disciplines);
														//console.log("keywords:", this.selectedInterface().keywords._latestValue);

														var discCheck = false;

														if (this.selectedInterface().displayName === "EBSCO Discovery Service"){
															$.each(this.selectedInterface().disciplines, function () {
																if (this.visible() || this.isDefaultSelected() === 'true') {
																	discCheck = true;
																}
															});
														...
														amplitude.getInstance().logEvent('Form - submitted',{
															interface: this.selectedInterface().displayName,
															is_keywords: this.selectedInterface().keywords._latestValue ? true : false ,
															keywords: this.selectedInterface().keywords._latestValue,
															is_discipline: discCheck
														});
		...
		$('#sbbMainContainer > *').not('#testDriveSection').on('click keyup change', function (e) {
			var tagName = e.target.tagName;
			//console.log("tagName",tagName);
			if (tagName === 'INPUT' || tagName === 'BUTTON' || tagName === 'IMG' || tagName === 'SELECT') {
				viewModel.buildSearchBox(true);
			}
		});
	}
