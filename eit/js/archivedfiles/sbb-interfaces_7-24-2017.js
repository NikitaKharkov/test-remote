...
var baseUrl = {
	logos: 'http://support.ebsco.com/images/logos/'
};
...
function ViewModel(configJson) {
	...
	var parseProxy = function () {
		console.log(self.selectedInterface().name);
			...
			buildSearchbox: function () {
				return {
					...
					!viewModel.profile.groupId())) {
						...
						if ($('#shib:checked').length && !viewModel.selectedInterface().custId().length) {
							errorMessage += 'Shibboleth requires your Cust ID to authenticate.\n';
							errorFlag = true;
						}
						if ($('#sso:checked').length && !viewModel.selectedInterface().custId().length) {
							errorMessage += 'Single Sign-on requires your Cust ID to authenticate.\n';
							errorFlag = true;
						}
						if ($('#athens:checked').length && !viewModel.selectedInterface().custId().length) {
