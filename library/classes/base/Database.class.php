    ...
    public function __construct($host, $name, $user, $password, $type)
    {
        ...
        if (($this->database_type == 'mysql')) {
            ...
            $this->connection = @pg_connect(
                'host='.$this->database_host.' user='.$this->database_user.' password=\''.$this->database_password.'\' dbname='.$this->database_name
            );
...
//              mssql_data_seek($result, $row);
        ...
        if ($this->database_type == 'mysql') {
            ...
            foreach ($tables as $table) {
                ...
                if ($this->numRows($result)) {
                    while ($row = $this->getRow($result)) {
                        ...
                        } elseif (stripos($row['Type'], 'float') !== false || stripos(
                                $row['Type'],
                                'double'
                            ) !== false || stripos($row['Type'], 'dec') !== false) {
            ...
            foreach ($tables as $table) {
                ...
                if ($this->numRows($result)) {
                    while ($row = $this->getRow($result)) {
                        ...
                        if (in_array($row['Type'], array('bigint', 'int', 'smallint', 'tinyint'))) {
                            ...
                            $output[$row['Field']]['default'] = ($row['DefaultVal'] === null) ? null : substr(
                                $row['DefaultVal'],
                                2,
                                strlen($row['DefaultVal']) - 4
                            );
                ...
                $sql = "
                    ...
                    WHERE CC.CONSTRAINT_CATALOG = '{$this->database_name}'
            ...
            foreach ($tables as $table) {
                ...
                $sql = "
                    SELECT pt.typname AS type,
                        pa.atttypmod  AS length,
                        pa.attname    AS field,
                        pg_get_constraintdef(pco.oid) AS CONSTRAINT, pa.attnotnull AS not_null, pd.adsrc AS DEFAULT
                    FROM pg_attribute AS pa
                        LEFT JOIN pg_class AS pc ON pa.attrelid = pc.oid
                        LEFT JOIN pg_type AS pt ON pt.oid = pa.atttypid
                        LEFT JOIN pg_constraint AS pco ON pco.conrelid = pc.oid
                            AND pa.attnum = ANY (pco.conkey)
                            AND (pco.contype = 'c' OR pco.contype = 'u')
                        LEFT JOIN pg_attrdef AS pd ON pc.oid = pd.adrelid AND pa.attnum = pd.adnum
                    ...
                    while ($row = $this->getRow($result)) {
                        ...
                        if (!in_array($row['type'], array('oid', 'cid', 'xid', 'cid', 'xid', 'tid'))) {
                            ...
                            if (stripos($row['default'], 'nextval(') !== false) {
                                ...
                                $output[$row['field']]['default'] = str_replace(
                                    "''",
                                    "'",
                                    preg_replace("/^'(.*)'::[a-z ]+$/i", '\1', $row['default'])
                                );
                            }
    ...
    public function getKeys()
    {
        ...
        if ($this->database_type == 'mysql' && $mysqli_version == 4) {
            ...
            if ($this->database_type == 'mysql') {
                    ...
                    if ($this->numRows($result)) {
                        while ($row = $this->getRow($result)) {
                            $found = preg_match_all(
                                '#FOREIGN KEY \(`([^`]+)`\) REFERENCES `([^`]+)` \(`([^`]+)`\)(?:\sON\sDELETE\s(SET\sNULL|SET\sDEFAULT|CASCADE|NO\sACTION|RESTRICT))?#',
                                $row['Create Table'],
                                $constraint_matches,
                                PREG_SET_ORDER
                            );
                ...
                $sql = "
                    SELECT KCU.TABLE_NAME          AS `table`,
                        ...
                        CASE
                            WHEN KCU.REFERENCED_TABLE_NAME IS NOT NULL THEN 'foreign_keys'
                    ...
                    WHERE KCU.CONSTRAINT_SCHEMA = '{$this->database_name}'
                      AND (
                            KCU.CONSTRAINT_NAME = 'PRIMARY'
                                OR (KCU.REFERENCED_TABLE_NAME IS NOT NULL AND KCU.REFERENCED_COLUMN_NAME IS NOT NULL)
                ...
                $sql = "
                    SELECT c.relname AS 'table',
                        ...
                        CASE
                            WHEN co.contype = 'f' THEN 'foreign_keys'
                    ...
                    FROM pg_attribute AS a
                        ...
                        LEFT JOIN pg_class AS rt ON co.confrelid = rt.oid
                        LEFT JOIN pg_attribute AS rf ON (rf.attnum = ANY (co.confkey) AND rt.oid = rf.attrelid)
                    WHERE (co.contype = 'p' OR co.contype = 'f' OR co.contype = 'u')
                ...
                $sql = "
                    ...
                    INNER JOIN INFORMATION_SCHEMA.KEY_COLUMN_USAGE AS kcu
                            ON c.table_name = kcu.table_name AND c.constraint_name = kcu.constraint_name
                    LEFT JOIN INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS AS rc
                            ON c.constraint_name = rc.constraint_name
                    LEFT JOIN INFORMATION_SCHEMA.CONSTRAINT_COLUMN_USAGE AS ccu
                            ON ccu.constraint_name = rc.unique_constraint_name
                    WHERE c.constraint_catalog = '{$this->database_name}'
                ";
            }
            ...
            if ($this->numRows($result)) {
                ...
                while ($row = $this->getRow($result)) {
                    ...
                    if ($row['key_type'] == 'unique_keys') {
                        ...
                        foreach ($row as $column => $value) {
                            if (!in_array($column, array('table', 'key_type', 'on_delete', 'constraint_name')
                                ) && !empty($value)) {
                                $temp[$column] = $value;
                            }
                            if ($column == 'on_delete') {
                ...
                foreach ($tables as $table) {
                    if (empty($output[$table]['foreign_keys'])) {
                        continue;
                    }
    ...
    {
        $this->printDebug(
            'Total time spent on queries was '.(number_format($this->debug_time, 5, '.', ',')).' seconds',
            true
        );
