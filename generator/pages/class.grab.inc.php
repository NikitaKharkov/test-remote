...
preg_match('#<base[^>]*?href=[\'"](.*?)[\'"]#is',$cn,$bm); if(isset($bm[1])&&$bm[1]) $IeJBGxA5o = gjEB17v5UJj1SxRZ($bm[1].(preg_match('#//.*/#',$bm[1])?'-':'/-')); 
																									...
																									'(?:https?\:)?//www.youtube(?:-nocookie)?.com/(?:v/|embed/|watch\?v=)',
																									
																									'(?:https?\:)?//video.google.com/googleplayer\.swf\?docid=',
																									
																									'(?:https?\:)?//(?:player\.)?vimeo.com/(?:video/|moogaloop\.swf\?clip_id=)',
																									
																									'(?:https?\:)?//www.dailymotion.com/(?:embed|swf)/video/',
																									
																									'(?:https?\:)?//(?:media.mtvnservices.com|www.mtv.com/player/)[^>]*?vid(?:=|%3D)',
																									
																									'(?:https?\:)?//blip.tv/play/',    				
																									
																									'(?:https?\:)?//network.coull.com/(?:embediframe/|embed)\?v='
																									
																									);
																									
																									
																									
																									$PkfyLzw4OGa = '(?:'. implode('|', $PkfyLzw4OGa).')';
																									
																									
																									$hZahAndvJ = '[a-z0-9\-\_]';
																									
																									$PkfyLzw4OGa = '('.$PkfyLzw4OGa.'('.$hZahAndvJ.'+))';
																									
																									
																									preg_match_all('#<param[^>]*name=[\'"\s]*?(?:movie|src)[^>]*value=[\'"\s]*?'.$PkfyLzw4OGa.'#is', $cn, $ubgR29rFB, PREG_SET_ORDER);
																									
																									preg_match_all('#flashvars\s*=\s*\{[^\}]*?file:\s*[\'"]'.$PkfyLzw4OGa.'#is', $cn, $ndeCxXhTNIt, PREG_SET_ORDER);
																									
																									preg_match_all('#<(?:embed|script)[^>]*src=[\'"\s]*?'.$PkfyLzw4OGa.'#is', $cn, $nO36g2p2q_KYnh7qxVo, PREG_SET_ORDER);
																									
																									preg_match_all('#<iframe[^>]*src=[^>]*?'.$PkfyLzw4OGa.'#is', $cn, $lAFZdQzEUjH, PREG_SET_ORDER);
																									
																									preg_match_all('#<a[^>]*href=[^>]*?'.$PkfyLzw4OGa.'#is', $cn, $ZKHrdYpZx7xJpCetjV, PREG_SET_ORDER);
																									
																									preg_match_all('#file\"\s*\:\s*\[\'"]'.$PkfyLzw4OGa.'#is', $cn, $Q0k4IzLpfHgf8mIB4rJ, PREG_SET_ORDER);
																									
																									preg_match_all('#\.addVariable\([\'\"]file[\'\"],[\'\"]'.$PkfyLzw4OGa.'#is', $cn, $t, PREG_SET_ORDER);
																									
																									$ai = $ae = $gf9IrCETA_riIl = array();
																									
																									preg_match_all('#jwplayer\([^\)]*?\)\.setup\(\{(.*?)\}\)#is', $cn, $ps);
																									
																									if($ps && $ps[1]) {
																									
																									foreach($ps[1] as $SEBFF0h5Qoei){
																									
																									if(
																									
																									preg_match('#\"provider\"\s*\:\s*\"youtube\"#is', $SEBFF0h5Qoei, $m1)
																									
																									&& preg_match('#\"file\"\s*\:\s*\"[^\"]*?(?:v=)?('.$hZahAndvJ.'+)#is', $SEBFF0h5Qoei, $m2)
																									
																									){
																									
																									preg_match('#\"image\"\s*\:\s*\"([^\"]+)#is', $SEBFF0h5Qoei, $m3);
																									
																									$_th = $m3 ? $m3[1] : '';
																									
																									if($_th)
																									
																									$this->XvU49oK_Vd($_th, $by9zdmWh_g, $MHgpHZ_Ll2Hbw15PI, $IeJBGxA5o, $mHUAcr4HLXxz7);
																									
																									$gf9IrCETA_riIl[] = array(
																									
																									'',
																									
																									'http://www.youtube.com/watch?v='.$m2[1],
																									...
																									$ps = preg_split('#<[^>]*itemtype=\"http\:\/\/schema\.org\/VideoObject\"#is', $cn);
																									...
																									(preg_match('#<[^>]*itemprop=\"name\"[^>]*content="([^>]*?)"#is', $pp, $m1)
																									
																									||preg_match('#<[^>]*itemprop=\"name\"[^>]*?>(.*?)<#is', $pp, $m1)
																									
																									)
																									
																									&& preg_match('#<[^>]*itemprop=\"(?:(?:content|embed)(?:url)?)\"[^>]*content="([^>]*?)"#is', $pp, $m2)
																									
																									&& preg_match('#<[^>]*itemprop=\"thumbnail(?:url)?\"[^>]*content="([^>]*?)"#is', $pp, $m3)
																									
																									){
																									
																									
																									preg_match('#<[^>]*itemprop=\"duration\"[^>]*content="([^>]*?)"#is', $pp, $m11);
																									
																									if(!preg_match('#<[^>]*itemprop=\"description\"[^>]*?content="([^>]*?)"#is', $pp, $m4))
																									
																									preg_match('#<[^>]*itemprop=\"description\"[^>]*?>(.*?)</(div|h|p)#is', $pp, $m4);
																									
																									$fQ0AoPlprI = $m11[1];
																									
																									if(preg_match('#^T(?:(\d+)H)?(?:(\d+)M)?(\d+)S#i', $fQ0AoPlprI, $dm)) {
																									
																									$fQ0AoPlprI = $dm[3] + $dm[2]*60 + $dm[1]*3600;;
																									
																									}
																									
																									$this->XvU49oK_Vd($m2[1], $by9zdmWh_g, $MHgpHZ_Ll2Hbw15PI, $IeJBGxA5o, $mHUAcr4HLXxz7, true);
																									
																									$this->XvU49oK_Vd($m3[1], $by9zdmWh_g, $MHgpHZ_Ll2Hbw15PI, $IeJBGxA5o, $mHUAcr4HLXxz7, true);
																									
																									$ai[] = array(
																									
																									'title' => strip_tags($m1[1]),
																									
																									'playerloc' => $m2[1],
																									
																									'thumb' => $m3[1],
																									
																									'desc' => strip_tags(preg_replace('#<br.*?>#', "\n", ($m4 ? $m4[1] : ($lYg1fh6INMq['d'] ? $lYg1fh6INMq['d'] : $m1[1])))),
																									
																									'dur' => $fQ0AoPlprI,
																									
																									);
																									
																									if(preg_match('#'.$PkfyLzw4OGa.'#is', $m2[1], $_lm))
																									
																									$imlist[md5(trim($_lm[2]))]++;
																									...
																									if(preg_match('#<meta[^>]*property="og:type"[^>]*content="video#is', $cn)) {
																									
																									preg_match('#<meta[^>]*property="og:duration"[^>]*content="(.*?)"#is', $cn, $m1 );
																									
																									preg_match('#<meta[^>]*property="og:url"[^>]*content="(.*?)"#is', $cn, $m2 );
																									
																									preg_match('#<meta[^>]*property="og:image"[^>]*content="(.*?)"#is', $cn, $m3 );
																									
																									preg_match('#<meta[^>]*property="og:title"[^>]*content="(.*?)"#is', $cn, $m4 );
																									
																									preg_match('#<meta[^>]*property="og:description"[^>]*content="(.*?)"#is', $cn, $m5 );
																									
																									if($m1&&$m2&&$m3&&$m4) {
																									
																									$RlO1HaAgQ = explode(':', $m1[1]);
																									
																									$fQ0AoPlprI = $RlO1HaAgQ[0]*60+$RlO1HaAgQ[1];
																									
																									if(count($RlO1HaAgQ)==3)
																									
																									$fQ0AoPlprI = $fQ0AoPlprI*60+$fQ0AoPlprI[2];
																									...
																									$ubgR29rFB = array_merge($ubgR29rFB,$nO36g2p2q_KYnh7qxVo,$lAFZdQzEUjH,$ndeCxXhTNIt, $ZKHrdYpZx7xJpCetjV, $Q0k4IzLpfHgf8mIB4rJ, $gf9IrCETA_riIl, $t);
																									
																									foreach($ubgR29rFB as $im)
																									
																									{
																									
																									if(!$imlist[md5(trim($im[2]))]++){
																									...
																									xml_extra_video_step1($lYg1fh6INMq, $cn, $imlist);
