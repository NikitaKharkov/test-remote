...
<form id="mainForm" method="post" action="mailer.php">
	<input type="hidden" id="req_Country" name="req_Country" value="<?=$_REQUEST["req_Country"]?>"/>
	...
	<?if($_REQUEST["req_Country"]=="Bolivia") {?>
		<fieldset id="onlineProducts" class="checks">
			<legend>EBSCO<em>host</em> Online Database Options</legend>
			<label for="prd_ASP"><input type="checkbox" id="prd_ASP" name="prd_Fuente_Academica" value="Fuente Academica"/>Fuente Academica</label>
			<label for="prd_BSP"><input type="checkbox" id="prd_BSP" name="prd_MedicLatina" value="MedicLatina<"/>MedicLatina</label>
		</fieldset>		
	<?} else {?>
		<fieldset id="onlineProducts" class="checks">
			<legend>EBSCO<em>host</em> Online Database Options</legend>
	...
	<p>IP address authentication is the most popular method our customers use. You can provide EBSCO
		...
		provide your ip addresses in the fields below if you'd like to use IP authentication.
	</p>
	<fieldset id="ipAuth">
