...
// Session array for rating start
...
$query = mysqli_query("SELECT total_votes, total_value, used_ips FROM $rating_dbname.$rating_tableName WHERE id='$id_sent' ")or die(" Error: ".mysqli_error());
...
$voted=mysqli_num_rows(mysqli_query("SELECT used_ips FROM $rating_dbname.$rating_tableName WHERE used_ips LIKE '%".$ip."%' AND id='".$id_sent."' "));
if(!$voted) {     //if the user hasn't yet voted, then vote normally...

	if (($vote_sent >= 1 && $vote_sent <= $units) && ($ip == $ip_num)) { // keep votes within range, make sure IP matches - no monkey business!
		$update = "UPDATE $rating_dbname.$rating_tableName SET total_votes='".$added."', total_value='".$sum."' WHERE id='$id_sent'";
...
$newtotals = mysqli_query("SELECT total_votes, total_value, used_ips FROM $rating_dbname.$rating_tableName WHERE id='$id_sent' ")or die(" Error: ".mysqli_error());
...
if ($vote_sent <3) {
//$new_back[] .= '<span class="thanks">Thank you for your feedback!</span>';
//$new_back[] .= '<p class="voted"> Rating: <strong>'.$vote_sent.'</strong> ';
//$new_back[] .= '<p class="voted">'.$id_sent.'. Rating: <strong>'.@number_format($sum/$added,1).'</strong>/'.$units.' ('.$count.' '.$tense.' cast) ';
//$new_back[] .= '<span class="thanks">Thank you for your feedback!</span></br></br>';
$new_back[] .= '<span class="thanks">We noticed you rated this a <span class="voted"><strong>'.$vote_sent.'</span></strong>.<br />
Please take a moment to let us know how we can improve this</span>
<iframe src ="/knowledge_base/comments.php?q='. $q .'&rating='. $vote_sent .'" width="600" height="180" frameborder="no" scrolling="no">
...
$new_back[] .= '<p class="voted"> Rating: <strong>'.$vote_sent.'</strong> ';
//$new_back[] .= '<p class="voted">'.$id_sent.'. Rating: <strong>'.@number_format($sum/$added,1).'</strong>/'.$units.' ('.$count.' '.$tense.' cast) ';
$new_back[] .= '<span class="thanks">Thank you for your feedback!</span></p>';
