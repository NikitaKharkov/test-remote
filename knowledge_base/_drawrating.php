...
//$query=mysqli_query($rating_conn, "SELECT total_votes, total_value, used_ips FROM $rating_dbname.$rating_tableName WHERE id='$id' ")or die(" Error: ".mysqli_error($rating_conn));

$query=mysqli_query($rating_conn, "SELECT kb_page_id FROM support_epnet.kb_pages WHERE kb_page_id ='$id' ")or die(" Error: ".mysqli_error($rating_conn));
...
if ($static == 'static') {
      ...
      for ($ncount = 1; $ncount <= $units; $ncount++) { // loop from 1 to the number of units
           if(!$voted && (isset($_COOKIE["PHPSESS"])) ) { // if the user hasn't yet voted, draw the voting stars
      ...
      $rater.='> <b>You have already voted. Thank you.</b>'; }
	  
	  elseif (!isset($_COOKIE["PHPSESS"])){ $rater.=' class="voted"'; 
      $rater.='> <b>Was this information helpful?</b><br /><span class="red">Please enable cookies and restart browser to vote.</span>'; }
	  
	  else {$rater.='> <b>Was this information helpful?</b>'; }
