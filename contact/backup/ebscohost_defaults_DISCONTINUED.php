...
$template->setHtmlTitle('EBSCOhost Defaults Opt-out');
...
<h1>EBSCOhost Defaults Opt-out</h1>
...
if ($page_function == "send") {
	...
	$validator->validateFields("institution_name, name, street, city, state_province, zip_code, country, email, phone");
	...
	if ($validator->getError()){
		...
		$today = date("Y-M-d");
		 	...
		 	$data = array($cust_id, $institution_name, $name, $street, $city, $state_province, $zip_code, $country, $email, $phone, $today);
		  			  
		  if ($fp = @fopen('/home/custadmin/ehost_defaults.csv', 'a')) {
		    fputcsv($fp, $data);
		    fclose($fp);
		    echo 'CSV written.';
		  } else {
		...
		$m = "";
		$m .="Support Website: (EBSCOhost Defaults Opt-out Request)\n\n";
		$m .="The following user has requested to opt-out of EBSCOhost Defaults:\n";
		$m .="-----------------------------------------------------------\n";
		...
		// Mail headers
		$date = date("m/d/Y");
		$mailsubject = "EBSCOhost Defaults Opt-out Request: $date";
...
if ($page_function == "show_form") {
	...
	if(!$message && !$error){
		...
		To opt out (indicate that you do NOT want the settings applied to your profiles),
		instead of automatically receiving the new default settings, simply complete the form below and then click on the <span class="red">Submit</span> button.
...
<div>
<form id="ebscohost_defaults" name="ebscohost_defaults" method="post" action="<?= $_SERVER['PHP_SELF']; ?>">
