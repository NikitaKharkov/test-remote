...
$template->printHeader();
	require_once($_SERVER['DOCUMENT_ROOT'] . "/contact/field_data_test.php");
...
if ($page_function == "send") {
	...
	if ($validator->getError()){
		...
		$EISServices = array(
			"EJS" => "assigned=314770&custevent_productlist2=1",
			"AtoZ" => "assigned=314770&custevent_productlist2=3",
			"LinkSource" => "assigned=47136&custevent_productlist2=35",
			"EBSCONET" => "assigned=18780&custevent_productlist2=4",
			"Book" => "assigned=18780&custevent_productlist2=6",
			"EIM" => "assigned=18780&custevent_productlist2=11");
		
		$querystring = "";
			
		if(in_array($_REQUEST['service'],array_keys($EISServices))) {
			//Dynamic values
			$values["email"] = stripslashes($_REQUEST['email']);
			$values["title"] = stripslashes($_REQUEST['subject']);
			$values["custevent_name"] = stripslashes($_REQUEST['first_name'])." ".stripslashes($_REQUEST['last_name']);
			$values["companyname"] = stripslashes($_REQUEST['institution_name']);
			$values["custevent_casetype"] = "2"; // 2=question
			...
			$values["incomingmessage"] .= "Occupation: " . stripslashes($occupations[ $_REQUEST['occupation'] ]) . "\n";
			$values["incomingmessage"] .= "Product: " . stripslashes($products[ $_REQUEST['product'] ]) . "\n\n";
			$values["incomingmessage"] .= stripslashes($_REQUEST['issue']);
			
			//Static values for EIS
			$values["h"] = "3590eea6826a0678c2d0";
			$values["compid"] = "405936";
			$values["formid"] = "8";
			...
			//add routing info
			$querystring .= $EISServices[ $_REQUEST['service'] ];
		}
		else {
			//Dynamic values
			$values["companyname"] = stripslashes($_REQUEST['institution_name']);
...
?>
Looking for an answer to a specific question? Send a message to our Customer Support Team. Please provide information for all required fields (marked with an asterisk).
...
<table align="center" cellpadding="8" cellspacing="0" class="form_table" border="0">
	<tr>
	...
	<tr>
		<td colspan="2">
			* How can we help you?<br />
			...
			<textarea name="issue" rows="10" cols="40" tabindex="10" /><?php echo $_POST['issue']; ?></textarea>
	...
	<tr>
		<td align="center" colspan="2">
			<input type="hidden" name="page_function" value="send" /><input type="image" src="/images/but_submit_form.gif" class="graphic_button" alt="Submit" tabindex="11" />
		</td>
	</tr>
</table>
</form>
